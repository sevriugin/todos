...
export const getWeb3 = (event) => {
    ...
    if (!w3) {
        let provider;
        if (event)
            provider = new Web3.providers.HttpProvider(Meteor.settings.contractEvents);
        else
            provider = new Web3.providers.HttpProvider(Meteor.settings.ethNodeAddress);
...
let pdk = undefined;
let defaultHdPathString = "m/0'/0'/0'";
